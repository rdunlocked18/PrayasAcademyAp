...
class _CategoryDetailScreenWidgetState
    ...
    if (state is LoadedCategoryDetailState) {
      return Column(
          ...
          children: <Widget>[
            Padding(
                padding:
                    const EdgeInsets.only(top: 30.0, left: 30.0, bottom: 5.0),
                child: Text(unescape.convert(selCat.name),
                    textScaleFactor: 1.0,
  ...
  _buildCourses(LoadedCategoryDetailState state) {
    return Padding(
        ...
        child: Container(
            //Edit For Making Coming Soon if no courses available
            child: state.courses == null || state.courses.length == 0
                ? Center(
                    child: Padding(
  ...
  _buildDropDownCategory(LoadedCategoryDetailState state) {
    return DropdownButtonHideUnderline(
      ...
      hint: Text(unescape.convert(selCat.name),
          textScaleFactor: 1.0, style: new TextStyle(color: Colors.white)),
      ...
      items: state.categoryList.map((Category catList) {
        return new DropdownMenuItem<Category>(
          ...
          child: new Text(unescape.convert(catList.name),
              textScaleFactor: 1.0, style: new TextStyle(color: Colors.black)),
        );
      }).toList(),
    ));
  }
